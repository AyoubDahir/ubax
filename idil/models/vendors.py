...
class Vendor(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get('opening_balance', 0.0) > 0.0:
            _logger.info("Invoking create_opening_balance_transaction during vendor creation for Vendor ID: %s",
                         vendor.id)
    ...
    def create_opening_balance_transaction(self):
        for vendor in self:
            if vendor.opening_balance > 0:
                try:
                    _logger.info("Attempting to create or update an opening balance transaction for vendor ID %s",
                                 vendor.id)

                    # Find the "Vendor Balance" transaction source
                    trx_source = self.env['idil.transaction.source'].search([('name', '=', 'Vendor Balance')], limit=1)
                    if not trx_source:
                        raise ValidationError(_("Transaction source 'Vendor Balance' not found."))
                    ...
                    if transaction:
                        ...
                        _logger.info("Updating existing opening balance transaction ID: %s", transaction.id)
                    else:
                        transaction_data['transaction_number'] = self.env['ir.sequence'].next_by_code(
                            'idil.transaction_booking')
                        transaction = self.env['idil.transaction_booking'].create(transaction_data)
                        _logger.info("Opening balance transaction created with ID: %s", transaction.id)
                    ...
                    _logger.error("Error occurred while creating or updating the opening balance transaction: %s",
                                  str(e))
                    raise ValidationError("Failed to create or update opening balance transaction: %s" % str(e))
    ...
    def _create_or_update_vendor_transaction(self, transaction, vendor):
        try:
            ...
            if vendor_transaction:
                ...
                _logger.info("Updated existing vendor transaction for Vendor ID: %s", vendor.id)
            else:
                self.env['idil.vendor_transaction'].create(transaction_data)
                _logger.info("Created new vendor transaction for Vendor ID: %s", vendor.id)

        except Exception as e:
            _logger.error("Error occurred while creating or updating vendor transaction for Vendor ID %s: %s",
                          vendor.id, str(e))
            raise ValidationError(
                "Failed to create or update vendor transaction for Vendor ID %s: %s" % (vendor.id, str(e)))
    ...
    def _update_or_create_booking_lines(self, transaction, vendor):
        try:
            ...
            if debit_line:
                ...
                _logger.info("Updated debit entry booking line for Vendor ID: %s", vendor.id)
            else:
                ...
                _logger.info("Created new debit entry booking line for Vendor ID: %s", vendor.id)
            ...
            if credit_line:
                ...
                _logger.info("Updated credit entry booking line for Vendor ID: %s", vendor.id)
            else:
                self.env['idil.transaction_bookingline'].create({
                    'transaction_booking_id': transaction.id,
                ...
                _logger.info("Created new credit entry booking line for Vendor ID: %s", vendor.id)

        except Exception as e:
            _logger.error("Error occurred while updating or creating booking lines for Vendor ID %s: %s", vendor.id,
                          str(e))
            raise ValidationError("Failed to update or create booking lines for Vendor ID %s: %s" % (vendor.id, str(e)))

    def write(self, vals):
        res = super(Vendor, self).write(vals)
        if 'opening_balance' in vals and vals.get('opening_balance', 0.0) > 0.0:
            for vendor in self:
                _logger.info("Invoking create_opening_balance_transaction during vendor update for Vendor ID: %s",
                             vendor.id)
                vendor.create_opening_balance_transaction()
        return res
